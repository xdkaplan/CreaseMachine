...
- **Real** — authored source-of-truth (mesh, `Pattern`/`PieceMap`, params, future
  ...
  **ownership tree** (§3), hold property values, and own **an optional geometry Transient** (a DAG downstream).
...
- **`Transient` — a dependency DAG.** Transients *derive*, and one can derive from **several**
  upstreams (a panel-layout from many Pieces + params), acyclically ⇒ **directed acyclic graph**.
  Fresh/stale. The DAG **hangs off** the Real-tree: Reals are the sources, Transients are
  everything computed from them.
- **Two vocabularies, one per graph (resolved 2026-06-24 — was an open overload):** the **Real
  tree** uses **Parent / Child** (*contains* / owned-by); the **dependency DAG** (a.k.a. the
  **Dependency Graph / Refresh Graph**) uses **Upstream / Downstream** (*derives-from* /
  flows-to). So a Transient has **upstreams** (what it derives from) and **downstreams** (what
  derives from it) — *never* "parents / children", which belong to the Real tree. Keeping the
...
(an upstream changed; cache can't be trusted). A plain `bool` — nothing richer (see §9 for why
`Status` is deferred).

**Core invariant — a Fresh node sits only over Fresh upstreams:** being Fresh guarantees
*everything it derives from (its upstreams) is Fresh too.* This is what makes lazy reads safe:
...
- **Rot** — Fresh → Stale. The invalidation. Cheap (a bool flip). **Cascades downstream.**
- **Refresh** — Stale → Fresh. Happens by one of two doors:
  ...
  - **Supply** — a **Supplied** transient is *fed* a value by a producer. Freshens self,
    rots downstreams. *(The `Supply` method — today a naked setter; named for the abstraction's
...
**The three rot-origin rules** — all the same primitive, `rotDownstream`, fired at three sites:

```
Real.mutate()        → rotDownstream(self)                       // a tx Apply/Invert
Transient.Rot()      → IsFresh=false; rotDownstream(self)        // the cascade
Transient.Supply(v)  → _value=v; IsFresh=true; rotDownstream(self)   // push: fresh self, stale downstreams

rotDownstream(n): foreach downstream d: d.Rot()   // and d.Rot() recurses → whole downstream sub-DAG
```

Because `Rot` itself calls `rotDownstream`, one `Rot()` at the top floods everything reachable
downstream — no separate "rot everything below" step. (In the DAG a node with several upstreams can be
...
"Grow rots downstreams"** rule: by the time a Grown transient grows, its downstreams were already
...
heavy compute is off-thread; only the microsecond `Supply` (assign + flip + `rotDownstream`)
...
worker: pass1 (low-res) → marshal → if gen==G: Supply(p1)   // UI: fresh + rotDownstream → render low-res
...
Progressive loading is just **several `Supply` calls** — each freshens self + rots downstreams,
...
reads must `Peek`), because you can't refresh past a not-yet-produced upstream.
...
**Upstream / Downstream** — the **dependency DAG** (Refresh Graph) axis: *derives-from* / flows-to; a Transient's *upstreams* are what it derives from, its *downstreams* what derive from it; rot walks downstream (§3, §5).
...
- ✓ **Resolved the Parent/Child overload (2026-06-24):** Real tree = **Parent / Child**; dependency
  DAG (Refresh Graph) = **Upstream / Downstream**. Swept through this spec. When the cascade is built, the
  designed `rotDownstream` primitive + any DAG-axis comments elsewhere adopt the same words.
...
- **"Element" retired → "Real."** This spec drops the Element class (a Real is the node; geometry is a
  Transient downstream; "viewable" = "has a geometry Transient"). The `AGENTS.md` (~:297) and
